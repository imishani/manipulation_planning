...
namespace ims {
    ...
    class scene3Dpoint : public ims::SceneInterface {
    public:
        explicit scene3Dpoint(std::shared_ptr<distance_field::PropagationDistanceField> &map_) : ims::SceneInterface() {
            this->df_map = map_;
        }
    ...
    struct actionType3Dpoint : public ims::actionType {

        actionType3Dpoint() : ims::actionType() {
            this->num_actions = 26;
    ...
    class actionSpace3Dpoint : public ims::actionSpace {
    ...
    public:
        actionSpace3Dpoint(const scene3Dpoint &env,
                           const actionType3Dpoint &actions_ptr) : ims::actionSpace() {
        ...
        bool getStateByValue(const stateType& state_val, size_t& state_ind) {
            ...
            auto* curr_state = new ims::state(state_val);
...
class DijkstraHeuristic : public ims::baseHeuristic{
public:
    ...
    void setGoal(ims::state* goal) override{
        mGoal = goal;
    ...
    bool run_planner(){
        auto* zero_heuristic = new ims::zeroHeuristic();
        ims::dijkstraParams params (zero_heuristic);
        ims::scene3Dpoint scene(m_distanceField);
        ims::actionType3Dpoint actions;
        action_space = std::make_shared<ims::actionSpace3Dpoint>(scene, actions);
        dijkstra_planner = new ims::dijkstra(params);
    ...
    bool getHeuristic(ims::state* s1, ims::state* s2, double& dist) override{
    ...
    bool getHeuristic(ims::state* s, double& dist) override{
    ...
    std::shared_ptr<ims::actionSpace3Dpoint> action_space;
    ...
    ims::dijkstra* dijkstra_planner = nullptr;

    std::string tip_link;
};


class BFSHeuristic : public ims::baseHeuristic{
public:
